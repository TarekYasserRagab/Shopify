...
class CartViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        arrangedCartArray = self.cartArray.reversed()
        tabItem.badgeValue = String(self.arrangedCartArray.count)
...
//MARK: - TableView
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return arrangedCartArray.count
    }

    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cartCell", for: indexPath) as! CartTableViewCell
     
        cell.productTitleLabel.text = arrangedCartArray[indexPath.row].itemName
        cell.productPriceLabel.text = arrangedCartArray[indexPath.row].itemPrice?.appending(" USD")

        let urlImage = NSURL(string: (self.arrangedCartArray[indexPath.row].itemImage ?? ""))
        let imagedata = NSData.init(contentsOf: urlImage! as URL)
             if imagedata != nil {
                 cell.productImageView.image = UIImage(data: imagedata! as Data)
        }

        cell.productQuantityLabel.text = String(arrangedCartArray[indexPath.row].itemQuantity)
        
//MARK: - Setup Functions for adding and substracting
        cell.addingItem = {
            self.cartViewModel.getSelectedItemInCart(productId: self.arrangedCartArray[indexPath.row].itemID) { selectedOrder, error in
...
//MARK: - Setup CartItemsFrom CoreData Functions
    ...
    func showDeleteAlert(indexPath:IndexPath){
        ...
        alert.addAction(UIAlertAction(title: "OK", style: .destructive, handler: { [self] UIAlertAction in
            cartViewModel.deleteFromCoreData(indexPath: indexPath, cartItems: arrangedCartArray)
            self.arrangedCartArray.remove(at: indexPath.row)
     
            self.tableView.reloadData()
            if self.arrangedCartArray.count == 0 {
               emptyCartImageView.isHidden = false
